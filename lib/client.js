...
`
  ...
  function TView(props) {
    ...
    const turns = React.useMemo(() => {
      ...
      for (const n of nodes) { if (n.kind === 'user') { if (cur.length > 0) { groups.push({ turn: turnIdx, nodes: cur, query: curQuery }); turnIdx++ }; cur = [n]; curQuery = sum(legText(n), 100) } else { cur.push(n) } }
